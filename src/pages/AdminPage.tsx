...
import type { Feedback, ModerationData, Profile, Report, UserReport } from '../lib/types';
...
export default function AdminPage() {
  ...
  const load = async () => {
    ...
    setBusy(true);
    try {
      const [userData, feedbackData, moderationData, userReportData] = await Promise.all([api.getUsers(token), api.getFeedback(token), api.getModeration(token), api.getUserReports(token)]);
      setUsers(userData); setFeedback(feedbackData); setModeration(moderationData); setUserReports(userReportData);
    } catch (error) {
